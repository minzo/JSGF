...
// 円と円の交差判定
//------------------------------------------------------------------------------
ColliObject.prototype.isHitCircleCircle = function( objA, objB ) {

    objA.transform( objA.bUpdateMatrix = false );
    objB.transform( objB.bUpdateMatrix = false );

    //  ObjAのCanvas座標を算出
    var vRdis= objA.size.x / 2;
    var vMtx = objA.matrix;
    var vCtr = MTX33.multVEC2( vMtx, { x:vRdis, y:vRdis } );

    //  衝突対象のCanvas座標を算出
    var uRdis= objB.size.x / 2;
    var uMtx = objB.matrix;
...
// 矩形と矩形の交差判定
//------------------------------------------------------------------------------
ColliObject.prototype.isHitRectRect = function( objA, objB ) {
...
// 円と矩形の衝突判定
//------------------------------------------------------------------------------
ColliObject.prototype.isHitCircleRect = function( circle, rect ) {

    circle.transform( circle.bUpdateMatrix = false );
    colli.transform( colli.bUpdateMatrix=false );

    //  自身のCanvas座標を算出
    var vPos = circle.pos;
    var vSize= circle.size;
    var vMtx = circle.matrix;
...
ColliCircle.prototype.isHitRect  = function( colli, callback ) {
    var isHit = this.isHitCircleRect( this, colli );
    if( isHit && callback != undefined) callback();
    return isHit;
};
ColliCircle.prototype.isHitCircle= function( colli, callback ) {
    var isHit = this.isHitCircleCircle( this, colli );
    if( isHit && callback != undefined ) callback();
    return isHit;
};
